...
if __name__ == "__main__":
    ...
    X.assign(count=y).to_csv(
        os.path.join(DATA_PATH, "storage", "bike_demand_train_features.csv"),
        index=False,
    )
    ...
    for i, params in enumerate(param_set):
        ...
        with mlflow.start_run(run_name=f"Run {i}"):
            ...
            pipeline = Pipeline(
                [("preprocessor", preprocess_pipeline), ("model", model)]
            )
            pipeline.fit(_X, y)

            # get evaluations scores
            score_cv = rmse_cv_score(model, X, y)
            ...
            # 로깅 정보 : 파라미터 정보
            mlflow.log_params({key: model.get_params()[key] for key in params})
            ...
            log_feature_importance(train=X, model=model)
